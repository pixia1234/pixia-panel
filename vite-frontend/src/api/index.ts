...
export const generateCaptcha = async () => {
  if (!axios.defaults.baseURL) {
    return { code: -1, msg: " - 请先设置面板地址", data: null };
  }
  const res = await axios.post(`/captcha/generate`, {}, { headers: { "Content-Type": "application/json" } });
  return res.data;
};
export const verifyCaptcha = async (data: { captchaId: string; trackData: string }) => {
  if (!axios.defaults.baseURL) {
    return { code: -1, msg: " - 请先设置面板地址", data: null };
  }
  const res = await axios.post(`/captcha/verify`, data, { headers: { "Content-Type": "application/json" } });
  return res.data;
}; 
